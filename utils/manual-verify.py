...
generated by Zig implementations for manual verification.
"""
...
def check_screenshot_exists(example_name):
    ...
    screenshots_dir = root_dir / "testing" / "results" / example_name
...
def find_examples_without_screenshots():
    """Find examples that don't have screenshots."""
    root_dir = Path(__file__).parent.parent
...
def build_example(example_name):
    ...
    # Build Zig example with -Dexample flag
    cmd = ["zig", "build", "-Dexample=" + example_name]
    try:
...
def cleanup_excel_file(example_name, zig_excel_file, prefix=""):
    """
    Move the Zig-generated Excel file to the results directory.
    ...
    """
    root_dir = Path(__file__).parent.parent
    results_dir = root_dir / "testing" / "results" / example_name
    ...
    # Destination path 
    dest_file = results_dir / f"{prefix}{example_name}{extension}"
    
    try:
        # Move the file
        shutil.move(str(zig_excel_file), str(dest_file))
        # Print relative path instead of full path
        relative_path = f"testing/results/{example_name}/{prefix}{example_name}{extension}"
...
def process_screenshot(screenshot_file, top_crop=25, bottom_crop=155, left_crop=0, right_crop=0):
    ...
    try:
        ...
        relative_path = str(screenshot_file).split(str(Path(__file__).parent.parent) + "/")[1]
        print(f"Error processing screenshot {relative_path}: {e}")
        return False


def take_screenshot(example_name, top_crop=25, bottom_crop=155, left_crop=0, right_crop=0):
    """
    Take a screenshot of the Excel file.
    
    This function:
    1. Builds the Zig example
    2. Runs the example to generate the Excel file
    3. Takes a screenshot using the macOS screencapture utility
    4. Opens the screenshot for the user to view
    5. Asks the user if the output looks correct
    ...
    screenshot_file = results_dir / f"comparison_{example_name}.png"
    ...
    print("Running example to generate Excel file...")
    try:
        subprocess.run(["zig-out/bin/" + example_name], cwd=root_dir, check=True)
    except subprocess.CalledProcessError:
    ...
    try:
        ...
        print(f"Warning: Failed to position Excel windows: {e}")
        print("Continuing with screenshot anyway...")
    
    # Take screenshot using screencapture
    relative_screenshot_path = f"testing/results/{example_name}/comparison_{example_name}.png"
    ...
    if screenshot_file.exists():
        ...
        # Ask user if output looks correct
        print("\n=== VISUAL VERIFICATION ===")
        print("Does the Excel output look correct? (y/n)")
        user_input = input("Enter y or n: ").lower()
        
        # Save verification result in the same directory as the screenshot
        ...
        with open(result_file, 'w') as f:
            ...
            if user_input == 'y':
                f.write("RESULT: VERIFIED - User confirmed output looks correct\n")
                print("✅ Output verified.")
                # Move the Excel file to results directory after verification
                if cleanup_excel_file(example_name, excel_file):
                    print("✅ Moved verified Excel file to results directory")
                else:
                    print("❌ Failed to move verified Excel file")
            else:
                f.write("RESULT: FAILED - User reported issues with output\n")
                print("❌ Output verification failed.")
                # Move the Excel file to results directory with 'failed-' prefix
                if cleanup_excel_file(example_name, excel_file, prefix="failed-"):
                    print("✅ Moved failed Excel file to results directory")
                else:
                    print("❌ Failed to move failed Excel file")
        
        # Create status file based on verification result
        status_file = results_dir / ("verified" if user_input == "y" else "failed")
        try:
            # Remove any existing status files
            if (results_dir / "verified").exists():
                (results_dir / "verified").unlink()
            if (results_dir / "failed").exists():
                (results_dir / "failed").unlink()
            
            # Create new status file
            status_file.touch()
            relative_path = f"testing/results/{example_name}/{'verified' if user_input == 'y' else 'failed'}"
            print(f"✅ Created status file: {relative_path}")
        except Exception as e:
...
def main():
    ...
    if args.cleanup:
        ...
        print(f"Moved {moved_count} of {len(zig_excel_files)} files to the results directory.")
        return 0
